...
You can decrypt captured payloads using api.newstargeted.com/admin/plugin-decrypt.php
"""
import json
import base64
import os

# Must match config.php PLUGIN_VERIFICATION_CIPHER_KEY
...
def decrypt_response(body_bytes, content_type='', expect_encrypted=False):
    ...
    try:
        ...
        # Encrypted: base64(iv).base64(ciphertext), or when we sent X-Encrypted expect it back
        is_encrypted = (
            expect_encrypted or
            ('text/plain' in content_type and '.' in body_str) or
            ('.' in body_str and body_str.strip()[0] not in '{[')
        )
