...
def _persist_activation_in_cyberpanel_db(request, activation_key):
    ...
    try:
        ...
        logging.writeToFile(f"Contabo Auto Snapshot: pluginHolder save_activation_key import failed: {str(e)}")
        return False
    ...
    for identity in identities:
        try:
            if save_activation_key(PLUGIN_NAME, identity, key_value, source='contabo_autosnapshot'):
                saved_any = True
        except Exception as e:
            logging.writeToFile(f"Contabo Auto Snapshot: save_activation_key failed for {identity}: {str(e)}")
    return saved_any
        ...
        try:
            ...
            if activation_key:
                try:
                    ...
                    try:
                        ...
                        logging.writeToFile(
                            f"Contabo Auto Snapshot: local activation DB verify result: ok={activation_ok} "
                        ...
                        activation_ok = False
                        logging.writeToFile(f"Contabo Auto Snapshot: local activation DB verify error: {str(_db_e)}")
            ...
            if not has_access and user_email and activation_key:
                try:
                    ...
                    logging.writeToFile(f"Contabo Auto Snapshot: has_saved_activation check error: {str(_hs_e)}")
