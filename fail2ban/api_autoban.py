...
def api_autoban_config(request):
    ...
    try:
        ...
        if config.enabled:
            ...
            # Immediate pass so enabling takes effect without waiting for interval
            try:
                banned = auto_ban.run_autoban_once()
                config = Fail2banAutoBanConfig.get_config()
                payload = _serialize_config(config)
                payload['just_banned'] = banned
            except Exception as e:
                logger.error('immediate autoban pass failed: %s', e)
                payload = _serialize_config(config)
                payload['just_banned'] = 0
        else:
            payload = _serialize_config(config)
