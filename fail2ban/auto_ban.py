...
When Fail2banAutoBanConfig.enabled is True, a single LSCPD worker periodically
bans alert IPs via fail2ban + firewall.
"""
...
def start_autoban_monitor():
    """Start one monitor thread per machine (cross-worker flock)."""
    ...
    with _monitor_lock:
        ...
        try:
            ...
            return
        _monitor_thread = threading.Thread(target=_monitor_loop, daemon=True, name='fail2ban-autoban')
        _monitor_thread.start()
        logger.info('fail2ban auto-ban monitor thread launched')


def ensure_autoban_monitor_if_enabled():
    try:
        from .models import Fail2banAutoBanConfig
        if Fail2banAutoBanConfig.get_config().enabled:
            start_autoban_monitor()
    except Exception:
        pass
