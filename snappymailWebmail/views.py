# -*- coding: utf-8 -*-
"""SnappyMail Webmail plugin admin settings."""
...
def admin_settings(request):
    ...
    context = {
        ...
        'plugin_name': 'SnappyMail Webmail',
        'version': '1.0.0',
        'is_paid': False,
        'status': status,
    }
    proc = httpProc(request, 'snappymailWebmail/admin_settings.html', context, 'managePlugins')
...
def api_toggle(request):
    try:
        ...
        return JsonResponse({
            'success': True,
            'message': 'SnappyMail webmail %s.' % ('enabled' if status['enabled'] else 'disabled'),
            'status': status,
        })
